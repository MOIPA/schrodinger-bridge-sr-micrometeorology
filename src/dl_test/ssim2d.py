...
def _uniform(window_size):
    uniform = torch.ones(window_size)
    return uniform / uniform.sum()


def _create_window(window_size: int, channel: int, sigma: float, use_gaussian):
    _win = _uniform(window_size)
    if use_gaussian:
        _win = _gaussian(window_size, sigma)
        logger.info("Gaussian window is created")
    else:
        logger.info("Uniform window is created")
...
class SSIM2D(torch.nn.Module):
    ...
    def scale_img(self, img: torch.Tensor) -> torch.Tensor:
        ret = (img - self.bias) / self.scale + self.offset
        return ret
